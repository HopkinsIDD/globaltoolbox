...
standardize_name <- function(location, scope, metadata, dbname=NULL, ...){
  ...
  db_scoped <- get_location_metadata(source=NULL, dbname=dbname, aliases=TRUE)
  ...
  names_b_data = db_scoped %>% select(id=id, name=names)
...
match_names <- function(name_a, names_b_data, 
  ...
  if (clean_a){
    ...
    name_a_nopunct <- str_replace_all(name_a, "[[:punct:]]", "") # remove all punctuation
  }
  ...
  for (j in 1:length(methods)){
    dists[,j]  <- suppressWarnings(stringdist(name_a_nopunct, names_b, method=methods[j]))
  }
